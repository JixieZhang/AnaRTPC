...
void drawline(int istart=0, int iend=2)
{
  ...
  ep->SetAlias("Row","int(StepID/20)");
  ep->SetAlias("Col","int(StepID%20)");
  ep->SetAlias("EventID","int(Index/25)");

  TCanvas *c21=new TCanvas("c21","",900,500);
  TH2F* hFrame=new TH2F("hIDTDCFrame","",120,0,120,550,0,550);
  ...
  for(int i=istart;i<iend;i++)
    {
      ...
      c21->cd();
      ep->Draw("CellID:TDC>>hIDTDC(120,0,120,550,0,550)",Form("StepID>=0 && EventID==%d",i),"text");
  ...
  for(int i=istart;i<iend;i++)
    {
      ...
      c22->cd();
      ep->Draw("CellID:TDCFold>>+hIDTDCFold(80,0,80,550,0,550)",Form("StepID>=0 && EventID==%d",i),"text");
      ...
      if(hIDTDCFold) 
	{
      ...
      gPad->Update();
      ep->Scan("(Index%25):HitNum:P0_p:Theta0_p*57.3",Form("HitNum>5 && EventID==%d",i));
