...
int  DisplayEvent( int entry=0, TCut extracut="", int ntrack=25)
{
  if(thisevent<=0) Init();
  ...
  c1->cd(3);
  int NHits = GetVariable("HitNum_m",DCcut);
  c1->cd(4);
  double Phi0 = GetVariable("Phi0_p*57.3",cut);

  int ShiftTDC = GetVariable("ShiftTDC",cut);
  ...
  h2yxframe->SetTitle(Form("Event %d: Track %d ShiftTDC=%d", iEvent/ntrack,iEvent%ntrack,ShiftTDC));
  ...
  if(!gROOT->IsBatch()) h2yxframe->Draw("same");
  gryx->Draw("same p");
  ep->Draw("StepY_rec_m:StepX_rec_m>>h2yx_rec",DCcut,"same*");

  c1->cd(2); h2yzframe->Draw();
  gryz->Draw("same p");
  ep->Draw("StepY_rec_m:StepZ_rec_m>>h2yz_rec",DCcut,"same*");

  c1->cd(3); h2xzframe->Draw();
  grxz->Draw("same p");
  ep->Draw("StepX_rec_m:StepZ_rec_m>>h2xz_rec",DCcut,"same*");

		
  c1->cd(4); h2rzframe->Draw();
  grrz->Draw("same p");
  ep->Draw("StepS_rec_m:StepZ_rec>>h2rz_rec_m",DCcut,"same*");

  c1->Update();
  c1->SaveAs(Form("Movie/RTPC_Event_%03d_track_%02d.png",iEvent/ntrack,iEvent%ntrack));

  ep->Scan("P0_p:Theta0_p*57.3:Phi0_p*57.3:Z0:HitNum_m:Smax:StepS[HitNum-1]:StepPhi[HitNum-1]:StepZ[HitNum-1]",cut);
